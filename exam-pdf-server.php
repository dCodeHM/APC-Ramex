...
// Fetch the CLOs using the syllabus_course_id
$sql = "SELECT clo_id, clo_number, clo_content FROM syllabus_clo WHERE syllabus_course_id = ?";
$stmt = $conn_soe->prepare($sql);
if (!$stmt) {
    die("Error preparing statement: " . $conn_soe->error);
}
...
function fetchRelatedQuestions($conn_ramex, $course_topic_id)
{
    ...
    $sql = "
    SELECT q.*, qc.question_choices_id, qc.answer_text, qc.answer_image, qc.is_correct, qc.letter, sc.clo_number
    FROM question q
    LEFT JOIN question_choices qc ON q.answer_id = qc.answer_id
    LEFT JOIN soe_assessment_db.syllabus_clo sc ON q.clo_id = sc.clo_id
