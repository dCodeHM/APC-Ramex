...
$_SESSION;
$conn_soe = mysqli_connect("localhost:3306", "root", "", "soe_assessment_db");
...
if (isset($_POST['update_admin_data'])) {
    $id = $_POST['account_id']; // Assuming 'user_id' is the name of the hidden input in the form

    // Sanitize and validate incoming data
    $role = mysqli_real_escape_string($conn_soe, $_POST['role']);

    // Prepare and execute the SQL update statement
    $query = "UPDATE account SET role=? WHERE account_id=?";
    $stmt = mysqli_prepare($conn_soe, $query);
...
if (isset($_POST['update_program_data'])) {
    ...
    // Sanitize and validate incoming data
    $role = mysqli_real_escape_string($conn_soe, $_POST['program_name']);
