...
function countExecutiveDirectors($conn_soe) {
    $execQuery = "SELECT COUNT(*) AS exec_count FROM account WHERE role = 'Executive Director'";
    $execResult = mysqli_query($conn_soe, $execQuery);
    if ($execResult) {
...
if (isset($_POST['input'])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $execDirectorCount = countExecutiveDirectors($conn_soe);
        ?>
                        ...
                        <tr>
                            ...
                            <td style="text-align: center">
                                <form action="adminsetcode.php" method="post">
                                    ...
                                    <select name="role" style="font-size: 10px;" class="rolebutton" <?= ($execDirectorCount <= 1 && $role === 'Executive Director') ? 'disabled' : '' ?> onchange="toggleProgramFields(this, '<?= $id; ?>')">
                                        <?php
                                        $sql = "SELECT * FROM role";
                                        $role_data = mysqli_query($conn_soe, $sql);
                            ...
                            <td style="text-align: center">
                                <form action="adminsetcode.php" method="post">
                                    <input type="hidden" name="account_id" value="<?= $id; ?>">
                                    <select name="program_name" class="rolebutton program-select-<?= $id; ?>" style="font-size: 10px;" <?= ($role === 'Professor') ? 'disabled' : '' ?>>
                                        <?php
                                        $sql = "SELECT * FROM program_name";
                                        $program_data = mysqli_query($conn_soe, $sql);
